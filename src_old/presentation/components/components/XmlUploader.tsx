...
import { importarXml, validateXmlSecurity, saveProcessedXml } from "@/core/services/xmlUtils
...
const XmlUploader: React.FC<XmlUploaderProps> = ({ onUploadSuccess }) => {
  ...
  const validateFile = async () => {
    ...
    try {
      ...
      const parsed = importarXml(content);
      setProgress(80);
  ...
  const processFile = async () => {
    ...
    try {
      ...
      setProgress(30);
      // Parse XML
      const parsedData = importarXml(content);
      setProgress(60);
      // Detectar se é padrão <Labels> e validar
  ...
  const loadSampleData = () => {
    setValidationStatus('validating');
    setProgress(30);
    // Função de exemplo removida: getSampleXmlData
    // Adapte aqui se quiser carregar dados de exemplo de outra fonte
    setTimeout(() => {
      try {
        // Exemplo: não faz nada
        setProgress(60);
        ...
        const parsedData = importarXml(sampleData);
